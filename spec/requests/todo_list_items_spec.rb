...
RSpec.describe "Todo List Items spec" do
  ...
  it "deletes a todo list item" do
    ...
    delete(todo_list_todo_list_item_path(todo_list, todo_list_item))
    expect(response.status).to eq 200
  ...
  it "edits a todo list item" do
    ...
    patch(todo_list_todo_list_item_path(todo_list, todo_list_item), params: { todo_list_item: { name: new_name } })
    expect(response.status).to eq 200
    get(todo_list_todo_list_item_path(todo_list, todo_list_item))
  ...
  it "creates a new todo list item" do
    ...
    post(todo_list_todo_list_items_path(todo_list), params: { todo_list_item: { name: name, is_done: is_done, due_date: due_date } })
