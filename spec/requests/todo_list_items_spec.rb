...
RSpec.describe "Todo List Items spec" do
  ...
  it "lists all todo list items" do
    ...
    get("/teams/#{team.id}/todo_lists/#{todo_list.id}/todo_list_items")
  ...
  it "shows a todo list item" do 
    ...
    get(team_todo_list_todo_list_item_path(team, todo_list, todo_list_item))
  ...
  it "deletes a todo list item" do
    ...
    delete(team_todo_list_todo_list_item_path(team, todo_list, todo_list_item))
    expect(response.status).to eq 200
  ...
  it "edits a todo list item" do
    ...
    patch(team_todo_list_todo_list_item_path(team, todo_list, todo_list_item), params: { todo_list_item: { name: new_name } })
    expect(response.status).to eq 200
    get(team_todo_list_todo_list_item_path(team, todo_list, todo_list_item))
